...
NEAT is a AI training and creating algorithm that creates a set of "genomes" and then trains them against each in various ways, from which we then select the best trained "genome" as the AI for whatever we require it for.

In this particular instance we are creating a Neural Network that will train the genomes with one another and then set a "fitness" value to it, the genome with the highest fitness rating will continue on to train with the next genome, and so on until the number of set generations are runned.

From this training spree we select the "best" genome and set it as the AI to control the Right Paddle of the ping pong game.
...
class pongAI:
    ...
    def test_ai(self, genome, config): #remove genome and config to train the AI
...
def eval_genomes(genomes, config):
    ...
    for i, (genome_id1, genome1) in enumerate(genomes):
        ...
        for genome_id2, genome2 in genomes[i+1:]:
            ...
            game.train_ai(genome1, genome2, config) #simulating the two AI and set their fitness


    #initialising the neat network to produce population to train
def run_neat(config):
    p =neat.Checkpointer.restore_checkpoint('neat-checkpoint-24') #(use this to restore a checkpoint)
    ...
    winner = p.run(eval_genomes, 52) #can change the number of genomes to set the number of generations
    #passing the number of generations to train a genome to while "winner" is the best of all of them
...
def test_ai(config): #testing the best AI
    width, height = 800, 600
    window = pygame.display.set_mode((width,height))
    ...
    game = pongAI(window, width, height)
    game.test_ai(winner, config) #running the test AI
...
if __name__ == "__main__":
    ...
    test_ai(config) #testing the best AI
